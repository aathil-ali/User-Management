...
terraform {
  ...
  backend "s3" {
    bucket         = "user-mgmt-terraform-state-staging"
...
locals {
  ...
  # Staging-specific configuration (production-like but smaller)
  config = {
    # Networking
    ...
    ecs_cluster_name    = "user-mgmt-staging"
    ...
    # Database - medium instances for staging
